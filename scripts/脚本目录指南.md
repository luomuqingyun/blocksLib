...
| **`utils/`** | **[工具]** 通用配置与辅助 | `data_sources.ts` (路径配置), `cleanup.js`, `maintain_boards.js` 等通用工具。 |
...
| :--- | :--- | :--- | :--- | :--- |
| **S1** | `core/7_parse_stm32duino_variants.ts` | `Arduino_Core_STM32/variants/**/boards_entry.txt` | `scripts/generated/stm32duino_support_registry.json` | 建立“官方支持白名单”，后续只处理在此名单中的芯片。 |
...
**逻辑**:
1. 读取基础数据，循环生成每个芯片的 JSON 文件。
2. 将外设功能、引脚映射注入到 JSON 中。
3. **[新增]** 读取 `stm32duino_support_registry.json`。
4. 对每个生成的芯片 ID (如 `generic_stm32f401re`)，进行模糊匹配查找其在 Registry 中的对应项。
5. 生成 `stm32_compatibility_enhanced.json`，供前端和 IDE 使用。
...
**数据获取原理详解**:
该脚本通过遍历 `Arduino_Core_STM32` 仓库中的每个 `boards_entry.txt` 文件来提取信息。
