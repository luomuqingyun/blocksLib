...
- **实时芯片视觉预览**：集成官方引脚映射数据，能够为 140+ 款 STM32 芯片渲染精确到物理位置的 SVG 交互式图表。
- **数据驱动的硬件引擎 (Board Data V2)**: 仅需 JSON 配置即可定义新板卡，支持动态引脚复用提示 (Hardware Hints) 与外设约束。
...
- **丰富的硬件模块**: 包含 `ChipInfo` (芯片信息)、`Basic I/O` (带定时器提示) 等核心积木组。
- **可扩展架构**:
