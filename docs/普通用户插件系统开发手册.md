...
### 8.5 芯片几何封装自动生成规范 (Chip Geometry Auto-generation)

如果插件未提供专门的 SVG 视图，软件会根据 `board.json` 中的 `package` 和 `pinCount` 自动生成标准的芯片示意图。

#### 1. 支持的 `package` 标准关键字 (不区分大小写)

| 分类 | 支持的关键字 | 渲染效果 |
| :--- | :--- | :--- |
| **四面引脚** | `LQFP`, `QFP`, `QFN` | 渲染为标准正方形芯片，引脚均匀分布在四个侧边。 |
| **双面引脚** | `TSSOP`, `SOIC`, `DIP`, `SOP` | 渲染为长方形芯片，引脚分布在左右（或长边）两侧。 |
| **网格阵列** | `BGA`, `WLCSP`, `LCC`, `LGA` | 渲染为点阵网格，支持 A1, B2 等坐标显示。 |

#### 2. 强制生效规则与拦截后果 (Important)

为了保证预览界面的工业美感并避免显示错误的图形造成误导，系统执行以下严格的拦截规则：

*   **规则 A：`pinCount` 必须准确且大于 0**
    *   绘图引擎完全依赖此数值来计算芯片的物理比例和引脚间距。
    *   **失败后果**：若缺失该字段或值为 `0`，预览区将被拦截并显示“封装信息不完整”遮罩。
*   **规则 B：`package` 必须匹配上述标准关键字**
    *   算法需要根据这些关键字决定渲染“正方形”还是“长方形”算法。
    *   **失败后果**：若使用了自定义名称（如 `My_Custom_Package`）或非法字符，由于无法匹配绘图路径，系统将自动拦截显示。
*   **规则 C：自定义视图提权 (CUSTOM_SVG)**
    *   如果你需要显示的不仅仅是一个几何芯片（例如：一块带有排母、USB 口、甚至实物照片的完整板卡），请使用此模式。
    *   **设置方法**：在 `board.json` 中明确设置 `"package": "CUSTOM_SVG"`。
    *   **文件命名**：在插件根目录下提供一个与 `board.json` 同名（仅扩展名不同）的 SVG 文件，例如：`board.svg`。
    *   **SVG 文件规范**：
        *   **尺寸**：建议使用 1:1 的正方形或长方形比例（如 400x400px）。
        *   **引脚交互**：普通用户预览图仅用于视觉展示，无需设置复杂的引脚 ID。如果需要高级交互，请在积木代码中处理逻辑。
        *   **内容限制**：系统支持标准的 SVG 元素，建议保持图形简洁，避免包含过大的位图以确保加载速度。
    *   **生效结果**：系统将完全跳过自动几何渲染，直接加载你的 SVG 文件作为新建项目预览图。
