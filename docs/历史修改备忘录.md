...
- **Fix**: 在 ExtensionRegistry 中引入了 originalManifest 备份机制和 languageChanged 事件监听。
- **Result**: 用户切换语言后，所有已加载插件的 UI 文本会自动刷新，无需重启。

#### 2. 全案例审计与修复
- **Extension**: 重构了 oard_esp8266, oard_micro, oard_yun 等官方示例，移除硬编码，全面对接 locales/。
- **Dev Module**: 修复了内部模块 	est_dev_block 的硬编码问题。
- **Status**: 目前所有示例工程及核心模块均达到 100% 国际化合规标准。


#### 5. 国际化插值语法修正 (I18n Interpolation Fix)
- **Bug**: 修复了导入/卸载提示中变量未正确替换的问题（显示为 {name}）。
- **Fix**: 将 i18next 占位符语法从单括号 {key} 修正为双括号 {{key}}。


#### 6. 市场添加输入框优化 (Marketplace UI Improvement)
- **Feature**: 在添加市场输入框旁增加了取消按钮。
- **UX**: 允许用户放弃添加操作并收起输入框，提升交互体验。


#### 7. 帮助文档打开方式优化 (Help Guide UX Improvement)
- **Feature**: 统一了插件市场指南的打开体验与帮助菜单一致。
- **Fix**: 默认使用内置 Markdown 浏览器打开，确保无需外部应用即可阅读，同时支持在外部打开以获得更好的阅读体验。
- **UI**: 移除了扩展列表为空时冗余的查看指南大按钮，仅保留顶部的常驻入口。


#### 8. 文档链接修复 (Documentation Link Fix)
- **Bug**: 修复了插件开发指南因文件重命名导致无法打开的问题。
- **Fix**: 更新后端 help:read-file 和 help:open-guide 接口，将目标指向新的 普通用户插件系统开发手册.md。


#### 9. 欢迎页文本优化 (Welcome Screen UI Polish)
- **UI**: 将打开项目保存目录精简为工作目录 (Work Directory)。
- **Desc**: 描述语从浏览你保存的项目文件优化为管理您的本地项目文件，更具生产力感。


#### 10. 扩展版本控制 (Smart Extension Versioning)
- **Feature**: 实现了完整的扩展版本管理系统。
- **UI**: 市场界面新增“更新”(绿色) / “已安装”(灰色) / “降级”(红色警告) 三种状态指示。
- **Logic**: 
    - 引入 Semver-lite 对比本地与远程版本。
    - **Update**: 点击更新按钮自动覆盖安装。
    - **Downgrade**: 自动拦截旧版本覆盖请求（无论是从市场还是本地导入），弹出二次确认对话框防止误操作，确认后支持 `force: true` 强制安装。
- **IPC**: 升级 `extensions:import` 和 `marketplace:install` 接口，支持传递 `force` 标志。

#### 11. 双仓库架构与自动化 (Split-Repo & Automation)
- **Infrastructure**: 将原有的 `blocksLib` 目录升级为 **Git Submodule**，链接至独立仓库 `luomuqingyun/blocksLib`。
- **Tools**:
    - `scripts/sync_repos.ps1`: 全能同步脚本。支持一键打包插件、更新子模块指针、联合提交主项目代码。
    - `blocksLib/pack_plugins.ps1`: 插件打包工具。自动遍历并压缩子文件夹为 ZIP 格式。
- **Docs**: 更新了开发指南，明确了双仓库协作的工作流。

#### 12. 自动化运维体系升级 (DevOps Automation Upgrade)
...
- **Sync Script**: 优化 `scripts/sync_repos.ps1`。
    ...
    - **Fix**: 修复了脚本文件的字符编码问题 (UTF-8)，增强了跨环境兼容性。
