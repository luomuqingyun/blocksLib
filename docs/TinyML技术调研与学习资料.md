...
## 1.5 关键释疑：Python 还是 C++? (FAQ)

> **Q: TinyML 通常用 Python 训练，但我们的项目生成 C++ 代码，这会有冲突吗？**

**A: 完全没有冲突，这是标准的工业流程。**

机器学习分为两个阶段：
1.  **训练阶段 (Python)**: 在电脑或云端（如 Colab, Edge Impulse）使用 Python (TensorFlow, PyTorch) 进行大数据计算，生成模型文件。
2.  **推理阶段 (C++)**: 将训练好的模型导出为 **C++ 字节数组 (Byte Array)** 或 FlatBuffer。MCU 只需要加载这个数组，调用 C++ 版的解释器 (TFLM) 即可运行。

**EmbedBlocks Studio 专注于第 2 步**。我们的积木会将那个“字节数组”和 C++ 推理引擎打包在一起，烧录进芯片。芯片上**不需要**运行 Python 解释器。
