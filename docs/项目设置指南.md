...
## 8. 全局应用配置 (Global Application Settings)

除了针对每个项目的配置，EmbedBlocks 还在全局层级维护了关键运行参数：

- **工作空间目录 (Work Directory)**:
    - **初次设定**: 自 v1.2.11 起，您可以在安装软件时直接指定默认的工作空间。
    - **自动检测**: 软件首次启动会智能识别安装时的路径。若未设置，将默认使用系统的“文档/EmbedBlocks”文件夹。
    - **修改**: 您可以随时在“设置 -> 常规”中更改工作空间。修改后，下一次“新建项目”将默认指向新路径。
- **界面语言**: 全局生效，支持即时切换。
- **调试开关**: 软件默认开启了 F12 开发者蓝图支持，方便高级用户排查底层问题。
