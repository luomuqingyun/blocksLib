...
// ====== 测试与 AI 注入流程注入 ======
// [性能优化]: 已移除顶部的静态 import，避免 Vite 静态分析爬取 1600+ 个前端积木文件
// 改为在运行参数中检测到测试/AI模式时，使用动态拼接路径的 require 方式加载
// ============================================================
...
function createWindow() {
    ...
    // ============================================================
    // [关键修复] Chromium Compositor/IME 焦点修复 (Focus Fix)
    // ============================================================
    // 根因：当 Electron 窗口失去 OS 焦点后，用户点击回来时，
    // Chromium 的 Compositor 和 IME 有时不自动恢复，
    // 导致 DOM focusable 元素虽然 active 但不显示光标。
    //
    // 三层防御策略：
    // 1. mainWindow.on('focus'): OS 级窗口获焦时同步 webContents
    // 2. IPC app:focus-fix: 渲染进程主动请求同步
    // 3. did-finish-load: 启动时确保 webContents 获焦

    // [防御 1] OS 级窗口获焦时，强制同步 Chromium Compositor
    mainWindow.on('focus', () => {
        mainWindow?.webContents.focus();
    });

    // [防御 2] 渲染进程主动请求 Compositor 同步（非破坏性）
    ipcMain.handle('app:focus-fix', async () => {
        if (!mainWindow) return;
        // 非破坏性：不 blur 窗口，只告诉 Chromium 同步焦点状态
        mainWindow.webContents.focus();
        // 如果上面不够，兜底：强制窗口获焦
...
app.whenReady().then(() => {
    ...
    if (isTestMode) {
        ...
        if (app.commandLine.hasSwitch('ai-create-project')) {
            // [性能优化]: 动态运行时 require，彻底躲过 Vite 的 AST 静态依赖追踪
            const aiRunnerName = 'aiRunner.js';
            const { runAiProjectCreation } = require(path.join(__dirname, aiRunnerName));
            ...
            // [性能优化]: 同上，使用路径拼接阻止 Vite 打包庞大的完整前端依赖
            const testRunnerName = 'testRunner.js';
            const { runTests } = require(path.join(__dirname, testRunnerName));
            runTests().then(() => {
                app.quit();
            }).catch((err: any) => {
